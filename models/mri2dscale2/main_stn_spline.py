...
# prepare data
n = hp["n_res_pts"]
inv_dat_file = '/home/ziruz16/pinn/data/datmri_dim2_n40000_st1_spline.mat'
xdat, udato, phi, pwm, pgm = map(n2t,read_mri_dat(n,inv_dat_file,DIM))
...
test_dat_file = '/home/ziruz16/pinn/data/datmri_dim2_n40000_st0_spline.mat'
xtest, utest, _, _, _ = map(n2t,read_mri_dat(n,test_dat_file,DIM))

bound = tf.reduce_max(rdat).numpy() # domain boundary[mm], maximum radius

udat = udato + tf.random.normal([n,1], 0, sigma, DTYPE)

# choose length scale
L = bound
T = L/np.sqrt(Dw_char*rho_char)
print(f'T = {T}, L = {L}')
scale = tf.constant([T]+[L]*XDIM,dtype=DTYPE)
