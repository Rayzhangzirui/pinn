...
mdirs = '/Users/Ray/project/glioma/pinn/models/numnuron/';
whichdir = '/Users/Ray/project/glioma/pinn/models/numnuron/n*'
...
%     export_fig(fullfile(m(i).modeldir,'fig_del_noise.jpg'),'-m3')
    ...
    [fig,ax1,ax2] = g.scatter('df',m(i).upred{end}.xdat(:,2:end), mksz, abs(m(i).upred{end}.upredxdat - m(i).upred{end}.udat),'filled','MarkerFaceAlpha',alpha);
    title(ax1,'abs u_{pred} - u_{end}')
    export_fig(fullfile(m(i).modeldir,'fig_upred_err.jpg'),'-m3')
end

%%
ns = arrayfun(@(x) x.info.num_hidden_unit, m);
[ns,idx] = sort(ns);
m = m(idx);
%%
close all
...
xlabel('n pts')
ylabel('%rel err')
title(sprintf('percentage rel err vs neurons per layer (%s)',optimizer{k}))
grid on
export_fig(fullfile(mdirs,sprintf('fig_err_npts_%s.jpg',optimizer{k})),'-m3')
...
close all
scatter(ns,log10(rateadam),'filled','DisplayName','adam')
hold on
scatter(ns,log10(ratelbfgs),'filled','DisplayName','lbfgs')
grid on
title('training speed')
legend('Location','best')
xlabel('n')
ylabel('log10 sec per 1k steps')
